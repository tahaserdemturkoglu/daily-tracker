...
def streak_count():
    ...
    # Bugün henüz veri yoksa dünden başla (sabah grace period)
...
def no_cache_headers(response):
    ...
    if (response.content_type and 'html' in response.content_type
            and b'</body>' in response.data and b'__sip2' not in response.data):
        response.data = response.data.replace(b'</body>', _V2_PATCH + b'</body>', 1)
    return response
...
def tg_stack_template_title(raw_text, actions):
    ...
    for pat in [r'ad[iı]\s+(.+?)\s+stack\s+olsun', r'ismi\s+(.+?)\s+stack\s+olsun', r'(.{2,50}?)\s+stackle']:
        ...
        if m:
            name = re.sub(r'\s+', ' ', m.group(1)).strip(" .,!?:;\"'")
            noisy = ['bunu', 'bunuda', 'bunu da', 'alindi', 'aldim', 'stack', 'eksik', 'icmedim', 'haric', 'hariÃ§']
